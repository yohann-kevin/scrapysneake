require "nokogiri"
require "open-uri"

class ScraperCourirController < ApplicationController

  def self.scrap_courir
    home_page_url = "https://www.courir.com/fr/c/homme/?prefn1=typology&prefv1=SNEAKERS"
    # home_page_url = "https://www.footlocker.fr/" scraper one page !
    # home_page_url = "https://www.chausport.com/" scraper one page !
    # home_page_url = "https://www.laboutiqueofficielle.com/" scraper one page !
    
    # site à scraper : 
    # https://www.cornerstreet.fr/sneakers/voir-tous-les-produits.html?p=3
    # https://www.footpatrol.fr/chaussures/
    # https://www.overkillshop.com/en/sneaker.html?p=2
    # https://www.fenom.com/fr/2-sneakers?page=2
    # https://www.urbanindustry.co.uk/collections/shoes?page=3
    # https://www.titoloshop.com/eu_en/men/sneakers?p=4
    # https://stockx.com/fr-fr/sneakers?page=4
    # https://www.flightclub.com/sneakers?page=12


    home_html = URI.open(home_page_url)
    home_page = Nokogiri::HTML(home_html)
    # find_most_wanted(home_page)
  end

  def self.find_most_wanted(home_page)
    # most_wanted = home_page.css(".most-wanted__container")
    # puts most_wanted
  end
end
